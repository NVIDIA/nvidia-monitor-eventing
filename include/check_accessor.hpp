...
class CheckAccessor
{
  public:
    ...
    /** constructor taking a resolved device pattern plus its index.
     *
     *  Needed when the caller already knows which device of a ranged
     *  'device_type' is being handled: accessor fields carrying a range are
     *  expanded against @a devIdData.index, which a device-name-only
     *  construction cannot supply.
     */
    explicit CheckAccessor(const util::DeviceIdData& devIdData) :
        _lastStatus(NotPerformed), _devIdData(devIdData)
    {
        // Empty
    }
